...
sidebar_current: "docs-bitbucket-resource-workspace-pipeline-runner"
description: |-
  Manages a self-hosted Bitbucket Pipelines runner for a workspace
---

# bitbucket\_workspace\_pipeline\_runner

Manages a self-hosted Bitbucket Pipelines runner at the workspace level.
...
```hcl
resource "bitbucket_workspace_pipeline_runner" "example" {
  workspace = "gob"
  name      = "linux-runner"
...
* `id` - The identifier of the runner in the form `WORKSPACE/RUNNER-UUID`.
...
Workspace pipeline runners can be imported using the workspace and runner UUID:

```sh
terraform import bitbucket_workspace_pipeline_runner.example gob/{12345678-90ab-cdef-1234-567890abcdef}
```
